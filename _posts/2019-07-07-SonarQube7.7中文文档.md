...
image: 'https://www.sonarqube.org/assets/img/SonarQube7.7中文文档/sonar-logo.svg'
...
原文：https://docs.sonarqube.org/7.7/
...
- 搜索节点由Amazon EC2 m4.2xlarge组成:8个vcpu, 32GB RAM-16GB分配给Elasticsearch
- 应用节点由Amazon EC2 m4构成。xlarge: 4个vcpu, 16GB RAM

#### 3.6.3 安装细节
准备您的个性化SonarQube包：

- 按照通常安装服务器文档的步骤安装SonarQube数据中心版本
- 安装您可能需要的所有其他插件，确认与数据库的连接工作正常。如果您手动执行此步骤，请确保使用插件版本矩阵检查与SonarQube版本的兼容性
- 压缩目录$SONARQUBE_HOME
- 现在您有了自己的个性化SonarQube数据中心版本包

在其他4个节点上部署SonarQube包：  
预期所有应用程序节点在硬件和软件方面都是相同的(相同的JVM构建)。类似地，所有搜索节点应该彼此相同。但是应用程序和搜索节点可能不同。通常，搜索节点配置的CPU和RAM比应用程序节点多。

- 从前面步骤的其他4个节点上解压缩您的个性化SonarQube包
- 祝贺您，现在您有了两个应用程序节点和三个搜索节点，它们在SonarQube软件方面完全相同
...
- 下载并解压您版本的SonarQube发行版到一个新的目录中，例如`$NEW_SONARQUBE_HOME`
- 手动安装与SonarQube版本兼容的非默认插件。使用兼容性矩阵确保您安装的版本与服务器版本兼容。注意，您的版本中所有SonarSource源代码分析器的最新版本都是默认安装的。不建议简单地将插件从旧服务器复制到新服务器;不兼容或重复的插件可能会导致启动错误。
- 更新`sonar.properities`和`wrapper.conf`文件内容(`$NEW_SONARQUBE_HOME/conf`)，根据`$OLD_SONARQUBE_HOME/conf`目录(web服务器URL、数据库、ldap设置等)中相关文件的设置。不要复制粘贴旧文件。如果您正在使用Oracle DB，请将其JDBC驱动程序复制到`$NEW_SONARQUBE_HOME/extensions/jdbc-driver/oracle`中。
- 停掉老的SonarQube服务
- 启动新的SonarQube服务
- 浏览器访问http://yourSonarQubeServerURL/setup并遵循安装说明
- 重新分析项目以获得新的数据

#### 3.8.2 其他需要关注的
##### 3.8.2.1 清理Oracle
从6.6开始，如果您正在使用Oracle，您可能需要执行另外一个步骤。在Oracle上，要删除的数据库列现在被标记为未使用，并且不再物理删除。要回收磁盘空间，Oracle管理员必须手动删除这些未使用的列。SQL请求是ALTER TABLE foo DROP未使用的列。相关的表列在系统表`all_unused_col_tabs`中。
...
如果您使用`InstallNTService.bat`将SonarQube安装为Windows服务，请在运行`$NEWSONARQUBEHOME的InstallNTService.bat`之前运行`$OLDSONARQUBEHOME/bin/…/UninstallNTService.bat`。
...
- 在所有语言中，“blame”数据将自动从受支持的SCM提供者导入。自动支持Git和SVN。其他提供程序需要额外的插件。
- 在所有语言上，都要执行源代码的静态分析(Java文件、COBOL程序等)。
...
- 在UI中定义的全局分析参数适用于所有项目(从顶部栏转到Administration > Configuration > General Settings（管理>配置>一般设置）)
- 在UI中定义的项目分析参数覆盖全局参数(在项目级别，转到Administration > General Settings（管理>通用设置）)
- 在项目分析配置文件或分析器配置文件中定义的项目分析参数可以覆盖在UI中定义的参数
- 启动分析时定义的分析/命令行参数覆盖项目分析参数
...
- 运行SonarScanner.MSBuild.exe begin命令，使用/d:propertyKey="path"语法指定报告可用的绝对路径("propertyKey"取决于工具)。
- 使用MSBuild构建您的项目
- 运行您的测试工具，指示它在先前指定给MSBuild SonarQube运行器的相同位置生成一个报告
...
|Python | sonar.python.pylint_config  sonar.python.pylint |默认情况下，如果您没有指定pylint报告的路径，SonarPython将为您执行pylint命令。使用sonar.python。在非默认位置使用pylint可执行文件。属性必须在windows上设置(即使是默认位置)，使用转义反斜杠或单斜杠(例如C:\\Python26\\Scripts\\pylint.bat, C:/Python26/Scripts/pylint.bat)。使用sonar.python。指定非默认pylint配置文件的相对或绝对路径。|
...
- 在待定任务的每一行可用的红色“x”上
- 在挂起的作业计数旁边的红色“批量取消”选项上。此按钮取消所有挂起的任务。
...
- 它们不能在SonarQube中管理;例如，没有能力将它们标记为假阳性。
- 在SonarQube中无法管理引发这些问题的规则的激活。事实上，外部规则在规则页面中不可见，也不反映在任何质量概要文件中。
...
- engineId - 字符串
- ruleId - 字符串
- primaryLocation - 位置对象
- type - 字符串。BUG，VULNERABILITY，CODE_SMELL中的一个
- severity - 字符串。BLOCKER, CRITICAL, MAJOR, MINOR, INFO中的一个
- effortMinutes - 整型。 可选的，默认值为0
- secondaryLocations - 位置对象数组，可选参数

位置字段：

- message - 字符串
- filepath - 字符串
- textRange - TextRange对象，仅可选用于辅助位置

TextRange 字段：

- startLine - 整型 1-indexed
- endLine - 整型，可选。 1-indexed
- startColumn - 整型，可选。 0-indexed
- endColumn - 整型，可选。 0-indexed
...
- lineNumber (必选)
- covered (必选)：布尔值，标识测试是否命中该行
- branchesToCover (可选)：可覆盖的分支数
- coveredBranches (可选)：测试实际覆盖的分支数量
...
- 自动分配问题
- 代码查看器中的代码注释(责备数据)
- scm驱动的新代码检测(帮助修复水泄漏)。在没有SCM数据的情况下，SonarQube使用分析日期(用于修改行的时间戳)来确定新代码。
...
- Git不认为旧的“Mac”行结束符(CR)是新行。因此，blame操作包含的行数将少于SonarQube的预期，分析将失败。解决方案是固定行结束使用Windows (CR/LF)或Unix (LF)行结束。
- JGit不支持.mailmap文件在被指责期间“清理”电子邮件地址
...
- 如果您在分析过程中得到了相同的错误，那么这看起来确实像是JGit中的一个bug(尤其是如果您没有使用本机git命令行工具时)。请尝试使用最新版本的JGit执行前面的步骤，并向SonarQube社区论坛报告所有信息。
...
您应该尝试在服务器上运行`svnadmin upgrade`。有关更多信息，请阅读*https://subversion.apache.org/docs/relea-notes/1.5.html#repos-updates*。
...
- 代码评审在这个过程中来得太晚了，没有利益相关者热衷于修复问题;每个人都想要发布新版本。
- 开发人员通常会拒绝不了解项目上下文的外部团队提出的建议。顺便说一下，正在审查的代码已经过时了。
- 这种方法明显缺乏对代码质量的所有权。谁拥有质量?没有人!
- 审查的是整个应用程序在投入生产之前，显然不可能将相同的标准应用于所有应用程序。每个项目都要进行谈判，这会耗尽整个过程的可信度。

相反，为什么不将您在家里使用的简单逻辑应用于管理代码质量的方式呢?修复泄漏意味着将重点放在“新”代码上，即自上次发布以来添加或更改的代码。然后事情就简单多了:

- 质量阈可以每天运行，通过它是可以实现的。在发布时没有什么意外。
- 对于开发人员来说，很难对他们前一天引入的问题进行反击。相反，他们通常乐于在代码仍然新鲜的时候修复问题。
- 代码质量有明确的所有权
- go/no-go的标准在应用程序之间是一致的，并且在团队之间共享。实际上，新代码就是新代码，不管它是在哪个应用程序中完成的
- 成本并不重要，因为它是开发过程的一部分
...
- 新的代码度量显示当前代码和您在其历史中选择的特定点(通常是previous_version)之间度量的差异
- 新代码主要是基于SCM的“指责”数据来检测的，这些数据从新代码周期(以前的“泄漏周期”)中的第一个分析开始，在需要时使用回退机制。有关更多细节，请参见SCM集成。
- 质量阈允许您设置布尔阈值，用于对代码进行度量。将它们与不同的度量标准一起使用，以确保随着时间的推移，代码质量朝着正确的方向发展。
...
- 阻断：影响生产中应用程序行为的高概率Bug:内存泄漏、未关闭的JDBC连接、…代码必须立即修复。
- 严重：要么是一个低概率影响生产中应用程序行为的bug，要么是一个表示安全缺陷的问题:空catch块、SQL注入、……必须立即检查代码。
- 主要：严重影响开发人员生产力的质量缺陷:未发现的代码段、重复的代码块、未使用的参数、…
- 次要：质量缺陷会对开发人员的工作效率产生轻微的影响:行不应该太长，“switch”语句应该至少有3种情况，…
- 提示：既不是缺陷也不是质量缺陷，只是一个发现。
...
- 如果问题在相同的规则上，具有相同的行号和相同的行散列(但不一定具有相同的消息)>匹配
- 检测块在文件中移动，然后如果问题在相同的(移动)行和相同的规则(但不一定是相同的消息)>匹配
- 在相同的规则上，使用相同的消息和相同的行散列(但不一定是相同的行)>匹配
- 在相同的规则上，使用相同的消息和相同的行号(但不一定使用相同的行散列)>匹配
- 使用相同的规则和相同的行散列(但不是相同的消息和不同的行)>匹配
- 是否存在匹配的已关闭问题>匹配并重新打开
...
- 对一个项目或分支的第一次分析
- 当概要文件中的规则是新规则时(激活的全新规则或已停用但现在激活的规则)
- 当分析器刚刚升级时(因为现在规则实现可以更智能)
- 当规则是外部的
...
- Confirm ：通过确认一个问题，你基本上是在说“是的，这是个问题。”这样做将把它从“打开”状态移到“确认”状态。
- 假阳性：从上下文来看这个问题，您会发现，不管出于什么原因，这个问题实际上并不是一个问题。所以你把它标记为假阳性，然后继续。要求管理项目上的问题权限。
- 无需解决：在上下文中查看这个问题，您会发现虽然它是一个有效的问题，但实际上并不需要修复。换句话说，它代表了可接受的技术债务。所以你标记它不会修复然后继续。要求管理项目上的问题权限。
- 严重程度的变化：这是前两个选项之间的中间地带。是的，这是一个问题，但它并不像规则默认的严重性所显示的那么糟糕。或者情况可能更糟。无论哪种方式，你都要调整问题的严重性，使之符合你认为它值得的程度。要求管理项目上的问题权限。
- 解决：如果你认为你已经解决了一个悬而未决的问题，你可以解决它。如果您是正确的，那么下一个分析将把它移动到关闭状态。如果你错了，它的状态会重新打开。

此外，安全热点还允许以下功能：

- 探测：确认安全热点为真实问题并手动打开漏洞。要求管理项目上的安全热点。
- 清理：将安全热点或手动打开的漏洞标记为没有问题，不应该修复。要求管理项目上的安全热点。
- 请求审查：要求安全审核员审查为纠正手动打开的漏洞所做的更改。
- 驳回：检查后，拒绝对手动打开的漏洞进行修复，并将其返回到打开的问题。要求管理项目上的安全热点。
...
- 语言:应用规则的语言。
- 类型:Bug、漏洞、代码气味或安全热点规则。
- 标记:可以向规则中添加标记，以便对它们进行分类并帮助更容易地发现它们。
- 存储库:为SonarQube提供规则的引擎/分析器。
- 默认严重性:规则的原始严重性——由提供此规则的分析器定义。
- 状态:规则可以有3种不同的状态:
    - Beta版:该规则最近才实施，我们还没有从用户那里得到足够的反馈，所以可能会出现误报或误报。
    - 弃用规则:不应该再使用该规则，因为存在类似但更强大和更准确的规则。 
    - 就绪:该规则已准备好在生产中使用。
- 可用日期:第一次在SonarQube上添加规则的日期。例如，列出自插件上次升级以来的所有新规则是很有用的。
- 模板:显示允许创建自定义规则的规则模板(请参阅本页面后面的内容)。
- 质量概要:包含在或排除在特定概要中
...
![alt 模板面](https://superslamzzz.github.io/assets/img/SonarQube7.7中文文档/6.6.4.png)
...
- 您必须能够使用CWE标识符搜索与CWE相关的规则。要在SonarQube平台中做到这一点，只需在规则页面的搜索文本输入中删除CWE标识符(例如CWE-595)并运行搜索。
- 规则必须与相关的CWE项目准确地链接。要查看SonarQube规则的CWE映射，请参阅规则描述底部的rule’s See部分。
- 您必须能够从问题中识别出相关的CWE。要在SonarQube平台上这样做，请参考相关规则。
- 产品文档必须包含对CWE和CWE兼容性的描述。
- 必须列出所支持的CWE版本。SonarQube语言插件支持2.8版本。
- 除了使用CWE id搜索规则外，还可以使用“CWE”规则标记进行搜索。
...
- brain-overload-一次要记住的东西太多了
- bad-practice-代码可能按照设计的方式工作，但是设计的方式被广泛认为是一个坏主意。
- cert-与CERT标准中的规则有关。目前有三种证书标准:C、c++和Java。其中许多规则不是特定于语言的，而是良好的编程实践。这就是为什么你会在非C/ C++， Java规则上看到这个标签。
- clumsy-额外的步骤用于完成一些可以更清晰、更简洁地完成的事情。(例如，在字符串上调用. tostring())。
- confusing-维护人员理解代码需要的时间会比代码实际执行的时间更长吗
- convention-编码约定——通常是格式化、命名、空格……
- cwe-与公共弱点枚举中的规则有关。有关CWE和一般安全相关规则的更多信息，请参见安全相关规则。
- design-代码的设计有些问题
- lock-in-使用特定于环境的特性
- misra-与MISRA标准中的一条规则有关。虽然MISRA规则主要是关于C和c++的，但其中许多规则并不是特定于语言的(例如，不要使用浮点数作为循环计数器)，而是简单的良好编程实践。这就是为什么您会在非C/ c++规则上看到这些标记。
- owasp-与OWASP十大安全标准中的一条规则有关。注意，OWASP Top Ten是一个高级漏洞列表，可以转换为许多潜在规则。
- pitfall-现在还没有什么问题，但是将来可能会出问题;已经为下一个家伙设好了陷阱，他很可能会掉进去，把代码搞砸。
- sans-top25-与SANS前25个编码错误有关，这些错误与安全性有关。注意，SANS Top 25列表是直接从CWE中提取的。
- suspicious-虽然不能保证这是一个bug，但是它看起来像一个可疑的bug。为了清晰起见，至少应该重新检查和重构代码。
- unpredictable-在当前条件下，代码可以很好地工作，但是如果条件发生变化，代码可能会不稳定地失败。
- unused-未使用的代码，例如从未使用的私有变量。
- user-experience-您的代码在技术上没有什么问题，但是它可能会让一些或所有用户讨厌您。
...
- 技术实现因应用程序而异(Web或Java应用程序的新代码可能不需要相同的代码覆盖率)。
- 您希望确保对某些应用程序(例如，内部框架)有更强的需求。
- 等等。。
...
#### 6.9.3 质量阈状态
当前状态在项目页面顶部突出显示:
![alt 质量阈状态](https://superslamzzz.github.io/assets/img/SonarQube7.7中文文档/6.9.3.png)
...
认识复杂度
理解代码的控制流有多难。请参阅[《 the Cognitive Complexity White Paper》](https://www.sonarsource.com/resources/white-papers/cognitive-complexity.html)，以获得用于计算该度量的数学模型的完整描述。
...
- 风险-可靠性和安全等级，测试覆盖率，技术债务和代码行
- 可靠性—可靠性评级、可靠性修复工作、代码行数和错误计数
- 安全性—安全性评级、安全性修复工作、代码行数和漏洞计数
- 可维护性——可维护性评级、技术债务、代码行数和代码气味计数
- 覆盖率——覆盖率、复杂性和未覆盖的行
- 重复-重复行%，代码行和重复块
- 在项目级别，Measures选项卡中提供了这些相同的可视化，以帮助您比较项目组件。项目概览对应于项目空间中的风险可视化，对于其他五个图，选择相关域中的Overview选项。
...
![alt 可视化示图](https://superslamzzz.github.io/assets/img/SonarQube7.7中文文档/6.13.2.png)

### 6.14 SonarLint智能通知
SonarLint智能通知可作为开发人员版或以上版本的一部分使用。  
智能通知允许开发人员使用SonarLint的连接模式接收来自SonarQube的ide通知时:
- IDE中打开的项目/解决方案的质量门状态(失败/成功)
- SonarQube分析提出了这个开发人员在IDE中开放的项目/解决方案中引入的新问题
...
- 您可能真的没有它们，因为代码是在没有使用任何安全敏感API的情况下编写的。
- 热点规则可能是可用的，但尚未在您的质量概要中激活，因此自然不会引发任何问题
- 更有可能的情况是，您使用的语言分析器还没有提供热点规则，因此它不会引发任何热点，而不管实际存在多少个热点，但是最后一个选项将随着时间的推移而消失。
...
- 运行代码分析时:用sonar中的令牌替换登录。登录属性。
- 调用web服务时:在执行基本身份验证时，只需传递令牌而不是您的登录。
...
- 浏览:访问一个项目，浏览它的度量、问题并执行一些问题编辑(确认/解决/重新打开、分配、评论)。
- 看源代码:查看项目的源代码。
...
- 必须首先通过单击Delete snapshot从项目历史记录中删除快照。它将不再显示在这个历史页面上，但仍然会出现在数据库中。
- 快照实际上在下一次项目分析期间被删除。
...
- 完全忽略一些文件或目录
- 将文件/目录排除在问题检测之外(特定规则或所有规则)，但分析所有其他方面
- 排除文件/目录的重复检测，但分析所有其他方面
...
![alt 忽略文件1](https://superslamzzz.github.io/assets/img/SonarQube7.7中文文档/7.3.1-1.jpg)
...
![alt 忽略文件2](https://superslamzzz.github.io/assets/img/SonarQube7.7中文文档/7.3.1-2.jpg)
...
![alt 忽略文件3](https://superslamzzz.github.io/assets/img/SonarQube7.7中文文档/7.3.1-3.jpg)
...
- 如果找到了第一个正则表达式，但没有找到第二个正则表达式，则认为文件的末尾就是块的末尾。
- 正则表达式不是在多行基础上匹配的。
...
- 忽略所有文件上的所有问题
    - KEY = *
    - PATH = **/*
- 忽略COBOL程序“bank/ZTR00021.cbl”上的所有问题
    - KEY = *
    - PATH = bank/ZTR00021.cbl
- 忽略直接位于Java包“com”中的类的所有问题。，但不在它的子包中
    - KEY = *
    -  PATH = com/foo/*
- 忽略针对“对象”目录及其子目录中的文件的编码规则“cpp:Union”的所有问题
    - KEY = cpp:Union
    - PATH = object/**/*
...
- 只检查“Bean”对象上的规则“Magic Number”，而不检查其他任何对象
    - KEY = checkstyle:com.puppycrawl.tools.checkstyle.checks.coding.MagicNumberCheck
    - PAYH = **/*Bean.java
- 只检查位于“bank/creditcard”和“bank/bankcard”目录中的“防止转到语句将控制转移到COBOL程序的当前模块之外”规则。这需要两个标准来定义:
...
##### 7.6.2.4 设置新的代码周期
新代码周期可以通过管理>一般设置>新代码>新代码周期来根据每个项目进行设置。默认值是`previous_version`，对于定期发布版本的项目来说，这是最合适的值。对于其他项目，例如30天(输入“30”，理解“days”部分)可能更合适。  
注意，这也可以在全局级别设置。

##### 7.6.2.5 更新项目的key值
可以在`Administration > Update key`上更新项目密钥(不丢失项目的历史记录)。  
...
### 7.7 网络连接
当项目分析完成时，webhook通知外部服务。包含JSON有效负载的HTTP POST请求被发送到每个URL。url可以在项目级和全局级指定。项目级别的规范并不替代全局级别的web挂钩。调用两个级别的所有钩子插件
...
- 技术实现因应用程序而异(例如，在构建线程或非线程Java应用程序时可能应用不同的编码规则)。
- 您希望确保对某些应用程序(例如，内部框架)有更强的需求。
...
- 机载认证和授权机制
- 强制用户在看到SonarQube实例的任何部分之前进行身份验证的能力
- 委托到身份验证的能力(更多信息请参见委托身份验证)

此外，您还可以在组或用户级配置以下人员:

- 查看曾经是否存在过这个项目
- 访问项目的源代码
- 管理一个项目(设置排除模式，为该项目调优插件配置，等等)
- 管理质量概要文件、质量门和SonarQube实例本身。
...
##### 8.2.1.1 身份验证机制
认证可以通过以下几种机制进行管理:
- 通过SonarQube内置的用户/组数据库
- 通过外部身份提供者，如LDAP服务器(包括Active Directory的LDAP服务)、GitHub等。参见插件库的身份验证和授权部分。
- 通过HTTP标头
...
两组有一个特殊的含义:
- Anyone是系统中存在的一个组，但是不能被管理。每个用户都属于这个组，包括匿名用户。
- sonar-users是自动添加用户的默认组。

##### 8.2.3.3 全局权限
要设置全局权限，请以系统管理员身份登录并转到Administration > Security >Global Permissions。

- Administer System:实例的所有管理功能:全局配置。
- Administer Quality Profiles：任何关于质量配置的行动。
- Administer Quality Gates: 对质量检验关采取任何行动
- Execute Analysis: 执行分析(项目、视图、报告、开发人员)，并获得执行分析所需的所有设置，甚至是scm帐户密码等安全设置。
- Create Projects: 在第一次分析之前初始化新项目的结构。在对尚未通过GUI 创建的项目进行第一次分析时也需要该权限。* **
- Create Applications: 创建一个新的应用程序.*  * *
- Create Portfolios: 创建一个新的质量概要文件.*  * *
...
- Administer Issues: 更改问题的类型和严重性，将问题解决为“无法修复”或“假阳性”(用户还需要“浏览”权限)。
- Administer Security Hotspots: 从“安全热点”“检测”(转换)漏洞，拒绝、清除、接受或重新打开“安全热点”(用户还需要“浏览”权限)。
- Administer: 访问项目设置并执行管理任务(用户还需要“浏览”权限)。
- Execute Analysis: 执行分析(项目、视图、报告、开发人员)，并获得执行分析所需的所有设置，甚至包括scm帐户密码、jira帐户密码等安全设置。

私有项目有两个额外的权限:  
- Browse: 访问一个项目，浏览它的度量、问题并执行一些问题编辑(确认/解决/重新打开、分配、注释)。
- See Source Code: 查看项目的源代码。
...
- 将密钥存储在SonarQube服务器上
    - 将生成的secred密钥复制到托管SonarQube服务器的机器上的文件中。默认位置是~/.sonar/sonar-secret.txt。如果你想把它储存在其他地方，设置它通过声纳的路径。secretKeyPath属性位于$SONARQUBE-HOME/conf/sonar.properties中
    - 将文件权限限制为运行SonarQube服务器的帐户(仅限所有权和读访问)。
    - 重启SonarQube服务器
- 生成您的设置的加密值
    - 回到管理>配置>加密，并使用已添加到接口的表单生成值的加密版本。

![alt 设置加密1](https://superslamzzz.github.io/assets/img/SonarQube7.7中文文档/8.2.5-1.jpg)

- 在SonarQube服务器配置中使用加密的值
...
- 拦截对SonarQube服务器的调用
- 处理身份验证
- 使用相关的SonarQube用户信息更新HTTP请求头
- 使用适当的头信息将请求重路由到SonarQube

![alt 报头身份1](https://superslamzzz.github.io/assets/img/SonarQube7.7中文文档/8.3.1-1.jpg)

激活和配置该特性所需的所有参数都可以在SonarQube服务器配置文件中找到(在$ SonarQube - home /conf/sonar.properties中)。  
使用Http报头身份验证是将SonarQube部署与内部SSO实现集成的一种简单方法。

#### 8.3.2 LDAP认证
通过在`$SonarQube-home/conf/sonar.properties`中配置正确的值，可以将SonarQube身份验证和授权配置到LDAP服务器(包括Active Directory的LDAP服务)。
主要特点是:

- 针对外部身份验证引擎进行密码检查。
- 自动同步用户名和电子邮件。
- 用户和组之间关系的自动同步(授权)。
- 能够根据外部和内部身份验证系统进行身份验证。如果LDAP服务器关闭，SonarQube内部系统会自动回退。
- 在第一次身份验证试验期间，如果用户的密码正确，SonarQube数据库将自动填充新用户。每次用户登录SonarQube时，在SonarQube数据库中刷新的用户名、电子邮件和用户所属的组。您也可以选择同步组成员关系，但这不是默认值。
...
##### 8.3.2.1 设置
- 通过编辑$SONARQUBE-HOME/conf/sonar配置LDAP插件。属性(见下表)
- 重启SonarQube服务器，检查日志文件:
> INFO org.sonar.INFO Security realm: LDAP ...  
> INFO o.s.p.l.LdapContextFactory Test LDAP connection: OK  
-  登录SonarQube
- 登出时，将向用户显示一个登录页面(/sessions/login)，用户可以通过传递适当的凭证选择作为技术用户或域用户登录  
...
#### 8.3.3 高级LDAP的话题
##### 8.3.3.1 认证方式
- Anonymous匿名——当绑定到LDAP服务器时，只需要对非受保护的条目和属性进行只读访问时使用。
- simple-对于不使用ldaps安全协议的生产部署，不推荐使用简单身份验证，因为它通过网络发送明文密码。
- CRAM-MD5 - 基于HMAC-MD5 MAC算法(RFC 2195)的挑战-响应认证方法(CRAM)。
- DIGEST-MD5 - 这是对cramm - md5身份验证方法(RFC 2831)的改进。
- GSSAPI - GSS-API是通用的安全服务API (RFC 2744)。GSS-API中最流行的安全服务之一是Kerberos v5，它在Microsoft的Windows 2000平台中使用。
...
- 详细的连接日志(以及从LDAP服务器接收到的潜在错误代码)输出到SonarQube的$SONARQUBEHOME/logs/web。log_，当日志记录处于调试模式时。
- 使用LDAP Java参数运行SonarQube分析时超时的记录如下:`http://docs.oracle.com/javase/jndi/tutorial/ldap/connect/config.html`。这些参数可以在`$SONARQUBE-HOME/conf/sonar.properties`文件中的`sonar.web.javaAdditionalOpts`进行设置。
- Kerberos故障诊断资源
    - 启用Kerberos日志
    - 故障排除Kerberos授权
- 排除NTLM的故障
...
- 在更新中心中，当您有挂起的更改时，restart按钮将显示在黄色横幅中(参见挂起的操作)
- 在任何时候的系统信息页面
...
#### 8.6.4 市场连接到哪些url
SonarQube市场连接到`https://update.sonarsource.org/`  以获得插件列表。大部分被引用的插件都是从以下网站下载的:  
`https://binaries.sonarsource.com/`  
`https://github.com/`

#### 8.6.5 通过代理连接应用市场
Marketplace使用HTTP(S)连接到外部服务器来提供这些服务。如果SonarQube位于代理之后，则必须在`$SonarQube-home/conf/sonar`中提供附加信息。属性配置文件:  
...
- 为每个项目:
    - 每天只有一个快照在1天后保存。不会删除由事件标记的快照。
    - 一个月后，每周只保留一张快照。不会删除由事件标记的快照。
    - 1年后，每个月只保留一个快照。不会删除由事件标记的快照。
    - 只有带有版本事件的快照才会在两年后保存。没有事件或只有其他事件类型的快照将被删除。
    - 删除所有大于5年的快照，包括由事件标记的快照。
- 所有30天以上的已关闭问题将被删除
- 删除包/目录级别的历史记录
...
要设置处理通知队列的频率，请在`$SONARQUBE-HOME/conf/sonar.properties`中设置`sonar.notification.delay`属性(以秒为单位)。必须重新启动服务器，才能考虑新值。
...
- 日志级别  
    ...
    - INFO – 默认级别
    - DEBUG - 对于高级日志。
    - TRACE - 显示高级日志和所有SQL和Elasticsearch请求。跟踪级别日志降低了服务器环境的速度，应该只用于跟踪web请求性能问题。
- 按进程划分的日志级别  
    通过以下属性，可以更精确地调整SonarQube服务器的4个进程的服务器端日志级别:
    - sonar.log.level.app – 对于SonarQube的主进程(也称为WrapperSimpleApp，启动其他3个进程的引导程序)
    - sonar.log.level.web – 设置web服务
    - sonar.log.level.ce – 设置计算引擎服务
    - sonar.log.level.es – 设置搜索引擎
- 日志迭代  
    要控制日志滚动，请使用sonar.log.rollingPolicy
    - time:[value] - 基于时间的旋转。例如，使用time:yyyy-MM-dd用于每日旋转，*time:yyyy-MM用于每月旋转。
    - size:[value] -基于对大小的旋转。例如,size:10MB。
    - none - 没有旋转。通常，当日志由外部系统(如logrotate)处理时，将使用这种方法。
...
每个Java进程都有自己的内存设置，可以在`$SONARQUBE-HOME/conf/sonar`中配置。属性文件。SonarQube附带的默认内存设置在大多数情况下都是可以的。如果你是支持大型SonarQube实例(超过100用户或超过5000000行代码)或一个实例持续集成管道的一部分,您应该监视所有三个关键的内存和CPU使用Java进程实例,以及整体的磁盘空间。监视将允许您查看是否有任何流程资源不足，并在资源短缺之前采取行动。有大量可用的监视工具(开源的和商业的)可以帮助您完成这项任务。SonarSource不推荐或支持任何特定的工具。

#### 8.10.2 内存设置
你可能需要增加你的记忆设置，如果你看到以下症状:
- 您的监视工具显示一个或多个SonarQube进程达到了内存限制
- 任何SonarQube进程崩溃并/或在sonar.log文件中生成内存不足错误
- SonarQube后台任务失败，后台任务日志中出现内存不足错误
- ElasticSearch实例的问题索引(在系统信息中可见)的大小大于或等于分配给ElasticSearch Java进程的内存

通过在`$SONARQUBE-HOME/conf/sonar.properties`中增加对应Java进程的-Xmx内存设置，可以增加分配给适当进程的最大内存：
- Web服务：sonar.web.javaOpts
- ElasticSearch服务：sonar.search.javaOpts
- Compute Engine服务：sonar.ce.javaOpts
...
注意：
- 自上次重新启动SonarQube以来处理的后台任务总数等于SuccessCount + ErrorCount
- 通过重新启动SonarQube，可以将这些值重置为默认值
